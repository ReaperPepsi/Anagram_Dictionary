...
# use this function to correlate the current sorted word with the key-word from DB
def sort_word(word: str) -> str:
    return "".join(sorted(word))


@router.post("/", response_model= AnagramPostResponse, status_code=status.HTTP_201_CREATED)
def group_anagrams(words: AnagramsInput, session: SessionDep, current_user: int = Depends(get_current_user)):
    list_of_added = []
    list_of_skipped = []
...
def delete_word(word: str, session: SessionDep, current_user_id: int = Depends(get_current_user)):
    # retreive the requested word from db
